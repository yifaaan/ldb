...
TEST_CASE("Can iterate breakpoint sites", "[breakpoint]")
{
    ...
    proc->BreakpointSites().ForEach([addr = 42](auto& site) mutable { REQUIRE(site.Address().Addr() == addr++); });

    cproc.BreakpointSites().ForEach([addr = 42](auto& site) mutable { REQUIRE(site.Address().Addr() == addr++); });
}
...
TEST_CASE("Range List", "[dwarf]")
{
    ...
    std::vector<std::uint64_t> rangeData{0x12341234, 0x12341236,
                                         // base address
                                         ~0ULL, 0x32, 0x12341234, 0x12341236, 0x0, 0x0};
