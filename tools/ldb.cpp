...
namespace
{
    ...
    std::string GetSignalStopReason(const ldb::Target& target, ldb::StopReason reason)
    {
        ...
        auto  message =
            fmt::format("stopped with signal {} at {:#x}", sigabbrev_np(reason.info), process.GetPc().Addr());
...
syscall <list of syscall IDs or names
    ...
    void PrintSource(const std::filesystem::path& path, std::uint64_t line, std::uint64_t n_lines_context)
    {
    ...
    void HandleStop(ldb::Target& target, ldb::StopReason reason)
    {
        ...
        if (reason.reason == ldb::ProcessState::stopped)
        {
            ...
            else if (auto entry = target.LineEntryAtPc(); entry != ldb::LineTable::iterator{})
            {
    ...
    void HandleBreakpointListCommand(ldb::Target& target)
    {
        ...
        target.Breakpoints().ForEach(
            [](const auto& bp)
            {
                ...
                else if (auto addr_bp = dynamic_cast<const ldb::AddressBreakpoint*>(&bp))
                {
                    fmt::print("address = {:#x}", addr_bp->Address().Addr());
                }
                fmt::print(", {}:\n", bp.IsEnabled() ? "enabled" : "disabled");
                bp.BreakpointSites().ForEach(
                    [](const auto& site)
                    {
    ...
    void HandleBreakpointSetCommand(ldb::Target& target, std::span<std::string_view> args)
    {
        bool hardware = false;
        if (args.size() == 4)
        {
    ...
    void HandleBreakpointToggle(ldb::Target& target, std::span<std::string_view> args)
    {
        auto command = args[1];
    ...
    void HandleBreakpointCommand(ldb::Target& target, std::span<std::string_view> args)
    {
        if (args.size() < 2)
        {
    ...
    void HandleWatchpointList(ldb::Process& process, std::span<std::string_view> args)
    {
        ...
        else
        {
            ...
            process.Watchpoints().ForEach(
                [&](const auto& watchpoint)
                {
                    fmt::print("{}: address = {:#x}, mode = {}, size = {}, {}\n", watchpoint.Id(),
                               watchpoint.Address().Addr(), StoppointModeToString(watchpoint.Mode()), watchpoint.Size(),
    ...
    void HandleSyscallCatchpointCommand(ldb::Process& process, std::span<std::string_view> args)
    {
        ...
        else if (args.size() >= 3)
        {
            ...
            auto toCatch = syscalls |
                std::views::transform(
                               [](auto syscall)
                               {
                                   return isdigit(syscall[0]) ? ldb::ToIntegral<int>(syscall).value()
