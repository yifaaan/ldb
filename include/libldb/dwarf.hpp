...
namespace ldb
{
    ...
    public:
        ...
        LineTable(Span<const std::byte> _data, const CompileUnit* _compileUnit, bool _defaultIsStmt,
                  std::int8_t _lineBase, std::uint8_t _lineRange, std::uint8_t _opcodeBase,
                  std::vector<std::filesystem::path> _includeDirectories, std::vector<File> _fileNames)
            : data(_data)
            , compileUnit(_compileUnit)
        ...
        const CompileUnit& Cu() const
        {
            return *compileUnit;
        }

        const std::vector<File>& FileNames() const
        {
            return fileNames;
        }
    ...
    public:
        ...
        const LineTable::Entry& operator*() const
        {
            return current;
        }
        const LineTable::Entry* operator->() const
    ...
    public:
        ...
        // Describes how to unwind from a specific function.
        ...
        struct EhFrameHeader
        {
            ...
            // Each entry contains: [function address, FDE address]
            const std::byte* search_table;
            // The count of entries in the search table.
            std::size_t count;
            // The encoding format for the entries.
            std::uint8_t encoding;
            // A pointer back to the parent call_frame_information object.
            CallFrameInformation* parent;

            // For efficient FDE lookup.
            const std::byte* operator[](FileAddr address) const;
        };

        CallFrameInformation(const Dwarf* dwawrf, const EhFrameHeader& hdr)
            : dwarf_{dwawrf}
            , eh_frame_header_{hdr}
        {
        }

        const Dwarf& dwarf() const
        {
            return *dwarf_;
        }
    ...
    public:
        ...
        const CallFrameInformation& call_frame_information() const
        {
            return *call_frame_information_;
        }
    ...
    public:
        ...
        const Abbrev* AbbrevEntry() const
        {
            return abbrev;
        }
