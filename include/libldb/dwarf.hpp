...
namespace ldb
{
	...
	class LineTable
	{
	  public:
		...
		LineTable(Span<const std::byte> _data, const CompileUnit* _compileUnit, bool _defaultIsStmt, std::int8_t _lineBase, std::uint8_t _lineRange, std::uint8_t _opcodeBase, std::vector<std::filesystem::path> _includeDirectories, std::vector<File> _fileNames)
			: data(_data),
	...
	class CallFrameInformation
	{
	  public:
		...
		CallFrameInformation(const Dwarf* parent, EhHdr eh_hdr) : dwarf_{parent}, eh_hdr_{eh_hdr}
		{
			eh_hdr.parent = this;
		}
	...
	class Die
	{
	  public:
		...
		Die(const std::byte* _position, const CompileUnit* _compileUnit, const Abbrev* _abbrev, std::vector<const std::byte*> _attrLocations, const std::byte* _next)
			: position(_position),
			  compileUnit(_compileUnit),
			  abbrev(_abbrev),
