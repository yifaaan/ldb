...
namespace ldb
{
    ...
    private:
        friend Target;
        FunctionBreakpoint(Target& _target, std::string _functionName, bool _isHardware = false,
                           bool _isInternal = false)
            : Breakpoint(_target, _isHardware, _isInternal), functionName(std::move(_functionName))
        {
            Resolve();
        }
    ...
    private:
        friend Target;
        LineBreakpoint(Target& _target, std::filesystem::path _file, std::size_t _line, bool _isHardware = false,
                       bool _isInternal = false)
            : Breakpoint(_target, _isHardware, _isInternal), file(std::move(_file)), line(_line)
        {
            Resolve();
        }
    ...
    private:
        ...
        AddressBreakpoint(Target& _target, VirtAddr _address, bool _isHardware = false, bool _isInternal = false)
            : Breakpoint(_target, _isHardware, _isInternal), address(_address)
        {
            Resolve();
        }
