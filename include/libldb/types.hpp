...
namespace ldb
{
    ...
    // the actual virtual addresses in the executing program after loading the ELF
    // file into memory
    class VirtAddr
    {
    public:
        ...
        // symbol table gives us a function’s address information in the form of file
        // addresses.
    ...
    public:
        FileAddr() = default;
        FileAddr(const Elf& elf, std::uint64_t _addr) : elf(&elf), addr(_addr)
        {
        }
    ...
    public:
        FileOffset() = default;
        FileOffset(const Elf& elf, std::uint64_t _offset) : elf(&elf), offset(_offset)
        {
        }
