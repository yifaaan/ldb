...
namespace
{
    ...
    std::unique_ptr<ldb::LineTable> ParseLineTable(const ldb::CompileUnit& cu)
    {
        ...
        auto version = cursor.U16();
        if (version != 4)
            ldb::Error::Send("Only DWARF 4 is supported");
        cursor.U32();
    ...
    ldb::CallFrameInformation::CommonInformationEntry ParseCie(Cursor cursor)
    {
        ...
        if (version == 4)
        {
            ...
            if (segment_size != 0)
                ldb::Error::Send("Invalid segment size");
        }
    ...
    ldb::CallFrameInformation::FrameDescriptionEntry ParseFde(const ldb::CallFrameInformation& cfi, Cursor cursor)
    {
        ...
        auto text_section_start = elf->GetSectionStartAddress(".text").value_or(ldb::FileAddr{});
        auto initial_location_address = ParseEhFramePointer(*elf, cursor, cie.fde_pointer_encoding,
                                                            current_offset.Offset(), text_section_start.Addr(), 0, 0);
    ...
    ldb::CallFrameInformation::EhFrameHeader ParseEhFrameHeader(const ldb::Dwarf& dwarf)
    {
        auto elf = dwarf.ElfFile();
        auto eh_header_start = *elf->GetSectionStartAddress(".eh_frame_hdr");
        auto eh_header_content = elf->GetSectionContents(".eh_frame_hdr");
        auto text_start = *elf->GetSectionStartAddress(".text");
    ...
    std::size_t EhFramePointerEncodingSize(std::uint8_t encoding)
    {
        ...
        case DW_EH_PE_udata8:
            return 8;
        default:
            ldb::Error::Send("Invalid pointer encoding");
        }
    }

    std::unique_ptr<ldb::CallFrameInformation> ParseCallFrameInformation(const ldb::Dwarf& dwarf)
    {
...
namespace ldb
{
    ...
    {
        if (auto it = std::ranges::find_if(compileUnits,
                                           [address](const auto& cu) { return cu->Root().ContainsAddress(address); });
    ...
    std::optional<Die> Dwarf::FunctionContainingAddress(FileAddr address) const
    {
        ...
        for (const auto& [name, entry] : functionIndex)
        {
            ...
            if (die.ContainsAddress(address) && die.AbbrevEntry()->tag == DW_TAG_subprogram)
                return die;
        }
        return std::nullopt;
    }
    ...
    std::vector<Die> Dwarf::InlineStackAtAddress(FileAddr address) const
    {
        ...
        if (func)
        {
            ...
            while (true)
            {
                ...
                auto found = std::ranges::find_if(child,
                                                  [=](auto& child)
                                                  {
    ...
    {
        if (address.ElfFile() != compileUnit->DwarfInfo()->ElfFile())
            return false;
    ...
    const CallFrameInformation::CommonInformationEntry& CallFrameInformation::GetCie(FileOffset at) const
    {
    ...
    const std::byte* CallFrameInformation::EhFrameHeader::operator[](FileAddr address) const
    {
        auto elf = address.ElfFile();
        auto text_section_start = *elf->GetSectionStartAddress(".text");
        auto encoding_size = EhFramePointerEncodingSize(encoding);
        // function address + FDE address
        ...
        while (low <= high)
        {
            ...
            auto current_entry_offset = elf->DataPointerAsFileOffset(cursor.Position());
            auto eh_header_offset = elf->DataPointerAsFileOffset(start);
            auto entry_address = ParseEhFramePointer(*elf, cursor, encoding, current_entry_offset.Offset(),
                                                     text_section_start.Addr(), eh_header_offset.Offset(), 0);
        ...
        // Get FDE address
        Cursor cursor{{search_table + high * row_size + encoding_size, search_table + count * row_size}};
        auto fde_offset = elf->DataPointerAsFileOffset(cursor.Position());
        auto eh_header_offset = elf->DataPointerAsFileOffset(start);
        auto fde_offset_int = ParseEhFramePointer(*elf, cursor, encoding, fde_offset.Offset(),
                                                  text_section_start.Addr(), eh_header_offset.Offset(), 0);
        FileOffset fde_file_offset{*elf, fde_offset_int};
    }
