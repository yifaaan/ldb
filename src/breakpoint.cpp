...
namespace ldb
{
    ...
    void AddressBreakpoint::Resolve()
    {
        ...
        {
            auto& newSite =
                target->GetProcess().CreateBreakpointSite(this, nextSiteId++, address, isHardware, isInternal);
    ...
    void FunctionBreakpoint::Resolve()
    {
        ...
        for (auto& die : foundFunctions.dwarfFunctions)
        {
            if (die.Contains(DW_AT_low_pc) || die.Contains(DW_AT_ranges))
            {
                ...
                {
                    auto& newSite = target->GetProcess().CreateBreakpointSite(this, nextSiteId++, loadAddress,
                                                                              isHardware, isInternal);
                    breakpointSites.Push(&newSite);
                    if (isEnabled)
                        newSite.Enable();
                }
        ...
        for (auto [elf, sym] : foundFunctions.elfFunctions)
        {
            ...
            {
                auto& newSite =
                    target->GetProcess().CreateBreakpointSite(this, nextSiteId++, loadAddress, isHardware, isInternal);
                breakpointSites.Push(&newSite);
                if (isEnabled)
                    newSite.Enable();
            }
