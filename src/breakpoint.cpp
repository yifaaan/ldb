...
namespace ldb
{
    ...
    void AddressBreakpoint::Resolve()
    {
        ...
        {
            auto& newSite = target->GetProcess().CreateBreakpointSite(this, nextSiteId++, address, isHardware, isInternal);
    ...
    void FunctionBreakpoint::Resolve()
    {
        ...
        for (auto& die : foundFunctions.dwarfFunctions)
        {
            if (die.Contains(DW_AT_low_pc) || die.Contains(DW_AT_ranges))
            {
                ...
                {
                    auto& newSite = target->GetProcess().CreateBreakpointSite(this, nextSiteId++, loadAddress, isHardware, isInternal);
                    breakpointSites.Push(&newSite);
                    if (isEnabled) newSite.Enable();
                }
            }
    ...
    void LineBreakpoint::Resolve()
    {
        ...
        for (auto& cu : dwarf.CompileUnits())
        {
            ...
            for (auto entry : entries)
            {
                ...
                auto shouldSkipPrologue = noInlineStack && (stack[0].Contains(DW_AT_ranges) || stack[0].Contains(DW_AT_low_pc)) && stack[0].LowPc() == entry->address;
