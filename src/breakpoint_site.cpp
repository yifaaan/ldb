...
namespace ldb {
BreakpointSite::BreakpointSite(Process& proc, VirtAddr addr, bool _isHardware,
                               bool _isInternal)
    : process(&proc),
      address(addr),
      isEnabled(false),
  ...
  id = _isInternal ? -1 : GetNextId();
}
BreakpointSite::BreakpointSite(Breakpoint* _parent, IdType _id, Process& proc,
                               VirtAddr addr, bool _isHardware,
                               bool _isInternal)
    : id(_id),
      process(&proc),
      address(addr),
...
void BreakpointSite::Enable() {
  ...
  if (isHardware) {
    ...
    errno = 0;
    std::uint64_t data =
        ptrace(PTRACE_PEEKDATA, process->Pid(), address, nullptr);
    if (errno != 0) {
