...
namespace ldb {
Elf::Elf(const std::filesystem::path& _path) : path(_path) {
...
const Elf64_Shdr* Elf::GetSectionContainingAddress(VirtAddr addr) const {
  for (auto& sh : sectionHeaders) {
    if (LoadBias() + sh.sh_addr <= addr &&
        addr < LoadBias() + sh.sh_addr + sh.sh_size) {
      return &sh;
    }
  }
  return nullptr;
...
std::optional<FileAddr> Elf::GetSectonStartAddress(
...
std::vector<const Elf64_Sym*> Elf::GetSymbolByName(
    std::string_view name) const {
...
std::optional<const Elf64_Sym*> Elf::GetSymbolContainingAddress(
    FileAddr addr) const {
...
void Elf::BuildSymbolMaps() {
  for (auto& sym : symbolTable) {
    ...
    int status;
    auto demangledName =
        abi::__cxa_demangle(mangledName.data(), nullptr, nullptr, &status);
    if (status == 0) {
    ...
    if (sym.st_value != 0 && sym.st_name != 0 &&
      ...
      auto addrRange =
          std::make_pair(FileAddr{*this, sym.st_value},
                         FileAddr{*this, sym.st_value + sym.st_size});
