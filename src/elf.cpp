...
namespace ldb
{
    ...
    {
        if (addr.ElfFile() != this || symbolAddrMap.empty())
            return std::nullopt;
    ...
    void Elf::ParseSectionHeaders()
    {
        ...
        std::copy(data + header.e_shoff, data + header.e_shoff + sizeof(Elf64_Shdr) * nHeaders,
    ...
    void Elf::ParseSymbolTable()
    {
        ...
        std::copy(data + symtab->sh_offset, data + symtab->sh_offset + symtab->sh_size,
    ...
    void Elf::BuildSymbolMaps()
    {
        for (auto& sym : symbolTable)
        {
            ...
            {
                auto addrRange =
                    std::make_pair(FileAddr{*this, sym.st_value}, FileAddr{*this, sym.st_value + sym.st_size});
