...
#include <cxxabi.h>
    std::string Target::FunctionNameAtAddress(VirtAddr address) const
    {
        ...
        if (func && func->Name())
        {
            return std::string{*func->Name()};
                 ...
                 elf_func && ELF64_ST_TYPE(elf_func.value()->st_value) == STT_FUNC)
        {
    ...
    Breakpoint& Target::CreateAdressBreakpoint(VirtAddr address, bool hardware, bool internal)
    {
        return breakpoints.Push(
            std::unique_ptr<AddressBreakpoint>(new AddressBreakpoint(*this, address, hardware, internal)));
    }

    Breakpoint& Target::CreateFunctionBreakpoint(std::string functionName, bool hardware, bool internal)
    {
        return breakpoints.Push(
            std::unique_ptr<FunctionBreakpoint>(new FunctionBreakpoint(*this, functionName, hardware, internal)));
    }
